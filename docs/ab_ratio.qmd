...
```{r}
extract_sp_coef <- function(fit_summary, trait_df, trait, round_a = 2) {
  t_s <- trait_df |> pull({{trait}}) |> scale()
  t_mean <- trait_df |> pull({{trait}}) |> mean()
  t_sd <- trait_df |> pull({{trait}}) |> sd()
  a_int <- fit_summary |>
  ...
  b_slope_re <- b_slope / t_sd
  list(
    a_int_re = a_int_re |> exp() |> round(round_a),
    a_power_re = a_power_re |> round(3),
    b_int_re = b_int_re |> round(3),
    b_slope_re = b_slope_re |> round(3))
}
...
draws_dh_sp <- withr::with_dir(rprojroot::find_root('_targets.R'),
    ...
    janitor::clean_names()
dh_sp_coef <- extract_sp_coef(summary_dh_sp, d_sp, log_dh, round_a = 3)
```
