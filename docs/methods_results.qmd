...
<We need a summary paragraph to describe what we wanted to do here>
...
xj <- tar_read(stan_data_no_xylem)$xj
tar_read(stan_data_no_xylem) |> str()
pca <- prcomp(xj[,-1])

d2 <- d |>
  filter(is.na(removed_k)) |>
  dplyr::select(wood_density, dh, vaf, vf, k) |>
  mutate(dh = log(dh), vaf = log(vaf), vf = log(vf), k = log(k)) |>
  filter(!is.na(dh)) |>
  filter(!is.na(vaf)) |>
  filter(!is.na(vf)) |>
  filter(!is.na(k)) |>
...
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_ks)$xj
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_vaf)
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_int)

tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_vf)
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_dh)
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_wood_density)
...
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_ks) |>
  filter(str_detect(variable, "tau"))

s1 <- tar_read(fit_abt2_summary_granier_with_traits_no_xylem_int) |>
  # filter(str_detect(variable, "tau_j_b|tau_k_b")) |>
  filter(str_detect(variable, "tau_A\\[2|tau_j_b")) |>
  # filter(str_detect(variable, "tau_A\\[2|tau_j_b|tau_k_b")) |>
  # filter(str_detect(variable, "|tau_j_b|tau_k_b")) |>
  # filter(str_detect(variable, "tau_j_b")) |>
  pull(q50)
s2 <- tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_vaf) |>
  # filter(str_detect(variable, "tau_j_b|tau_k_b")) |>
  filter(str_detect(variable, "tau_A\\[2tau_j_b")) |>
  # filter(str_detect(variable, "tau_A\\[2|tau_j_b|tau_k_b")) |>
  # filter(str_detect(variable, "tau_j_b")) |>
  pull(q50)
(sum(s1^2) - sum(s2^2)) / sum(s1^2)

s1 <- tar_read(fit_abt2_summary_granier_with_traits_no_xylem_int) |>
  # filter(str_detect(variable, "tau_j_a|tau_k_a")) |>
  filter(str_detect(variable, "tau_A\\[1|tau_j_a|tau_k_a")) |>
  pull(q50)
s2 <- tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_ks) |>
  # filter(str_detect(variable, "tau_j_a|tau_k_a")) |>
  filter(str_detect(variable, "tau_A\\[1|tau_j_a|tau_k_a")) |>
  pull(q50)
(sum(s1^2) - sum(s2^2)) / sum(s1^2)
...
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_ks) |>
  filter(str_detect(variable, "tau_j_a"))
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_ks) |>
  filter(str_detect(variable, "tau_A"))

v1 <- 0.539^2 + 0.234^2 + 1.04^2
v2 <- 0.303^2 + 0.094^2 + .099^2 + 0.15^2 + 0.42^2
...
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_vaf)
tar_read(fit_abt2_summary_granier_with_traits_no_xylem)

ggplot(d2, aes(vaf, vf)) +
  geom_point()
...
tar_read(vaf_pred_data_no_xylem)

tar_read(ks_pred_data_no_xylem)
...
pca3 <- prcomp(tmp, scale = TRUE)

library(targets)
library(tidyverse)
tar_read(stan_data_no_xylem_test_log_ks)
...
d2 <- d |>
  ...
  summarize(
    log_ks = log(mean(ks)),
    log_vaf = log(mean(vaf))) |>
ungroup()

tmp <- tibble(x = rnorm(1000), y = rnorm(1000))

ggplot(tmp, aes(x, y)) +
  geom_point() +
  geom_smooth(method = "lm")

ggplot(d2, aes(x = log_ks, y = log_vaf)) +
  geom_point()

vaf <- targets::tar_read(stan_data_no_xylem_log_vaf)$xj
ks <- targets::tar_read(stan_data_no_xylem_log_ks)$xj
plot(vaf[,2], ks[,2])

targets::tar_read(stan_data_no_xylem_test) |> str()

tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_ks)$xj
tar_read(fit_abt2_summary_granier_with_traits_no_xylem_log_vaf)$xj
...
Can you make these clean?

p_a2 <- d2 |>
  ggplot(aes(x = a1_q50, y = a2_q50, col = xylem_long_fct)) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  geom_point() +
  geom_errorbar(aes(ymin = a2_q2_5, ymax = a2_q97_5), alpha = 0.5) +
  geom_errorbar(aes(xmin = a1_q2_5, xmax = a1_q97_5), alpha = 0.5) +
  scale_x_log10() +
  scale_y_log10() +
...
p_a3 <- d3 |>
  ggplot(aes(x = a1_q50, y = a3_q50, col = xylem_long_fct)) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  geom_point() +
  geom_errorbar(aes(ymin = a3_q2_5, ymax = a3_q97_5), alpha = 0.5) +
  geom_errorbar(aes(xmin = a1_q2_5, xmax = a1_q97_5), alpha = 0.5) +
  scale_x_log10() +
  scale_y_log10() +
  xlab("a - traditional fitting") +
...
p_a4 <- d4 |>
  ggplot(aes(x = a1_q50, y = a4_q50, col = xylem_long_fct)) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  geom_point() +
  geom_errorbar(aes(ymin = a4_q2_5, ymax = a4_q97_5), alpha = 0.5) +
  geom_errorbar(aes(xmin = a1_q2_5, xmax = a1_q97_5), alpha = 0.5) +
  scale_x_log10() +
  scale_y_log10() +
  xlab("a - traditional fitting") +
...
p_b2 <- d2 |>
  ggplot(aes(x = b1_q50, y = b2_q50, col = xylem_long_fct)) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  geom_point() +
  geom_errorbar(aes(ymin = b2_q2_5, ymax = b2_q97_5), alpha = 0.5) +
  geom_errorbar(aes(xmin = b1_q2_5, xmax = b1_q97_5), alpha = 0.5) +
...
p_b3 <- d3 |>
  ggplot(aes(x = b1_q50, y = b3_q50, col = xylem_long_fct)) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  geom_point() +
  geom_errorbar(aes(ymin = b3_q2_5, ymax = b3_q97_5), alpha = 0.5) +
  geom_errorbar(aes(xmin = b1_q2_5, xmax = b1_q97_5), alpha = 0.5) +
  xlab("b - traditional fitting") +
  ylab("b - model 3") +
  my_theme() +
...
p_b4 <- d4 |>
  ggplot(aes(x = b1_q50, y = b4_q50, col = xylem_long_fct)) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  geom_point() +
  geom_errorbar(aes(ymin = b4_q2_5, ymax = b4_q97_5), alpha = 0.5) +
  geom_errorbar(aes(xmin = b1_q2_5, xmax = b1_q97_5), alpha = 0.5) +
  xlab("b - traditional fitting") +
  ylab("b - model 4") +
  my_theme() +
...
ab_each_tmp <- function(data, x_var, y_var, x_label, y_label, use_log_scale = TRUE) {
  ...
  if (use_log_scale) {
    ...
    p <- p + coord_cartesian(xlim = c(0.5, 2), ylim = c(0.5, 2))
  }
...
# Individual plots
p_a2 <- ab_each_tmp(d2, "a1", "a2", "a - traditional fitting", "a - model 2")
p_b2 <- ab_each_tmp(d2, "b1", "b2", "b - traditional fitting", "b - model 2", use_log_scale = FALSE)
p_a3 <- ab_each_tmp(d3, "a1", "a3", "a - traditional fitting", "a - model 3")
p_b3 <- ab_each_tmp(d3, "b1", "b3", "b - traditional fitting", "b - model 3", use_log_scale = FALSE)
p_a4 <- ab_each_tmp(d4, "a1", "a4", "a - traditional fitting", "a - model 4")
p_b4 <- ab_each_tmp(d4, "b1", "b4", "b - traditional fitting", "b - model 4", use_log_scale = FALSE)
...
tar_make(ab_points_four_models_plot_all)
tar_make(ab_points_four_models_plot)

s1 <- tar_read(fit_ab_each_sap_sp_clean_0.08) |>
  filter(species == "Hevea brasiliensis")
...
s1$fit_segments[[1]]$draws |>
  posterior::as_draws_df() |>
  dplyr::select(log_a, b)

tar_read(test2_summary_granier_without_traits_full_segments_no_xylem)
tar_read(test4_summary_granier_without_traits_full_segments2) |>
  filter(str_detect(variable, "tau"))

tar_read(test4_summary_granier_without_traits_full_segments3)

loo1 <- tar_read(test2_mcmc_granier_without_traits_full_segments_no_xylem)$loo(cores = parallel::detectCores())
loo2 <- tar_read(test4_mcmc_granier_without_traits_full_segments2)$loo(cores = parallel::detectCores())
loo3 <- tar_read(test4_mcmc_granier_without_traits_full_segments3)$loo(cores = parallel::detectCores())

tar_load(fit_ab_draws_granier_without_traits_full_pool_sap_all_clean_0.08)
generate_post_ab(fit_ab_draws_granier_without_traits_full_pool_sap_all_clean_0.08)

library(tidyverse)
library(targets)
library(data.table)
tar_load(dir_dep_imp_full_df)
tar_load(dbh_imp_df)
tar_load(post_ab_pool_mc4)
post_ab_pool_mc <- tar_read(post_ab_pool_mc4)
tar_load(post_ab_segments_mc4)
tar_load(post_slen)
tar_load(post_dir_dep)
...
library(tictoc)
tic()
tmp1 <- generate_ab_uncertainty(
        dir_dep_imp_full_df |> sample_n(200000),
        dbh_imp_df,
        post_ab_pool_mc = post_ab_pool_mc4,
        post_ab_segments_mc = post_ab_segments_mc4,
        post_slen, post_dir_dep, k = 2000, i = 1 ) |>
        mutate(pg = 0.08)
toc()

tic()
tmp2 <- generate_ab_uncertainty(
        dir_dep_imp_full_df |> sample_n(200000),
        dbh_imp_df,
        post_ab_pool_mc = tar_read(post_ab_pool_mc2_1_0.08_fit_ab_draws_granier_without_traits_full_pool_sap_all_clean_0.08_fit_ab_draws_granier_without_traits_full_segments_sap_all_clean_0.08),
        post_ab_segments_mc = tar_read(post_ab_segments_mc2_1_0.08_fit_ab_draws_granier_without_traits_full_pool_sap_all_clean_0.08_fit_ab_draws_granier_without_traits_full_segments_sap_all_clean_0.08),
        post_slen, post_dir_dep, k = 2000, i = 1 ) |>
        mutate(pg = 0.08)
toc()

data.frame(a = colnames(tmp1), b = colnames(tmp2))

hoge |>
   filter(str_detect(name, "uncertainty_df_1_0.08")) |> pull(command)
...
tar_read(ab_uncertainty_full_df) |> str()
tar_read(ab_uncertainty_full_each_df) |> str()
