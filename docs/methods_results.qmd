...
Specifically, the explained variance for a component (i.e., species and pressure) was computed as the ratio of its variance to the total variance, expressed as a percentage.
For instance, the percentage of variance explained by species was calculated as:

$$
\frac{\sigma^2_{sp}}{\sigma^2_{sp} + \sigma^2_{pres} + \sigma^2_{res}} \times 100\;(\%).
$$

where $\sigma^2_{sp}$, $\sigma^2_{pres}$, and $\sigma^2_{res}$ are variance for species, pressure, and residuals, respectively.
...
```{stan, file=here::here('stan/no_temporal.stan'), echo=TRUE, eval=FALSE, output.var="amam"}
```

## sapwood length

```{stan, file=here::here('stan/normal.stan'), echo=TRUE, eval=FALSE, output.var="m;asjdf"}
```
...
summary_sep$fit_segments[[1]]$summary |> pull(variable)
summary_sep$fit_pool[[1]]$summary |> pull(variable)

tar_read(pool_ab_table_sap_sp_clean_0.08)

pg <- 0.08
       tmp <-  str_c("without_traits_fit_ab_summary_granier_without_traits_full_pool_sap_all_clean_",
        pg,
        "_data.without_traits_pool_",
        pg, ".csv")
...
pg = c(
  seq(0.02, 0.08, by = 0.01),
  seq(0.02, 0.04, by = 0.005),
  seq(0.05, 0.08, by = 0.01)
)

tar_read(segments_ab_table_sap_sp_clean_0.08)
...
hoge <- tar_read(segments_ab_table_full) |>
...
d1 <- read_csv("data/species_only_post.csv")
d2 <- read_csv("data/segments_inclusive_post.csv")


tar_read(without_traits_table_fit_ab_summary_granier_without_traits_full_pool_sap_all_clean_0.035_0.035)

d1 |>
  filter(effective_sample_size < 400)

hoge <- tar_manifest()
hoge |>
  filter(str_detect(name, "segments_xylem_table"))

  filter(str_detect(name, "without_traits_table"))




tar_read(species_xylem_table_fit_ab_summary_granier_without_traits_full_pool_sap_all_clean_0.02_0.02)

tar_read(segments_xylem_table_fit_ab_summary_granier_without_traits_full_segments_sap_all_clean_0.025)

tar_read(species_xylem_table_fit_ab_summary_granier_without_traits_full_pool_sap_all_clean_0.025) |> as.data.frame()
