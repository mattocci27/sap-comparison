...
While we employed a power-law function as a model form, we used two different Bayesian model approaches to analyze the relationship between *F*~d~ and K.
We call this approach the "multilevel approach."
On the other hand, our second approach followed a more conventional route.
Here, species serve as the unit of analysis, and inter-segment variation is not considered.
We have labeled this method as the "pooled approach."

## Results

The power-law fittings without (i.e., traditional approaches) and with (i.e., multilevel approaches) inter-segment variation yielded similar estimates for coefficients *a* but slightly different estimates for coefficients *b* (Figs. `r s_fig_num("pool_multi")`-`r s_fig_num("ab_points")`; Table S4).
Generally, the estimates of coefficients *a* and *b* tended to increase with increasing maximum applied pressure gradients (Fig. S`r s_fig("pg_multi")`-`r s_fig("pg_ribbon_b")`).
Note that the larger the maximum applied pressure gradients, the larger the sample sizes for the analyses.
Our multilevel model revealed little variance in the estimates of coefficient *a* and *b* between species (`r fig("coef_density")` and Table S5).
Xylem types (80.77% of total variance) were the largest source for the variance for coefficient *a*.
In comparison, segments (40.26% of total variance) were the largest source for the variance for coefficient *b* (Table S5).
