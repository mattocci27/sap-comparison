...
We also examined the effect of pressure types and species on *K~s~* using a Bayesian ANOVA model (SI xxx).
We quantified the proportion of total variance attributed to each component in our model.
Specifically, the explained variance for a component (i.e., species and pressure) was computed as the ratio of its variance (i.e., $\sigma^2_{sp}$ and $\sigma^2_{pres}$) to the total variance, expressed as a percentage.
For instance, the percentage of variance explained by species was calculated as:

$$
\frac{\sigma^2_{sp}}{\sigma^2_{sp} + \sigma^2_{pres} + \sigma^2} \times 100\;(\%).
$$

## Pressure vs. number of active vessels

We modeled the probability of a vessel filled with silicon as linear and quadratic logistic functions of perfusion pressures with varying- intercepts and slopes by species (SI xxx).
...
Using a power-law function as the foundational model form, we applied four different Bayesian model approaches to analyze the relationship between *F*~d~ and K.
...
First, the species-specific model takes individual species as its primary analytical unit without considering inter-segment variations.
In this approach, each species was fitted separately.
This model is the most conventional approach [(SI model)](hyperlink-to-model-code).

Second, the segment-separate fit model uses segments as its primary analytical unit.
While this model incorporates both inter-segment and interspecific variations, each species was fitted separately [(SI model)](hyperlink-to-model-code).

Third, the species-pooled model takes species as its primary analytical unit, integrating interspecific and xylem-type variations [(SI model)](hyperlink-to-model-code).
Unlike the previous models, this model fits all species simultaneously.

Finally, the segment-pooled model is based on segments but also includes variations between segments, species, and xylem types.
Similar to the species-pooled model, this model fits all species together, serving as another key model in our study [(SI model)](hyperlink-to-model-code).
...
Each of these traits was included separately as species-level predictors for coefficients *a* and *b*, and were integrated into our segment-pooled model, both with and without consideration for xylem type variation [(SI model)](hyperlink-to-model-code).

## Scaling

(**First, we need a summary paragraph to describe what we wanted to do here.
We might also need a conceptual diagram to explain the analtical framework.**)
...
We used the *K* values measured at a 0-2 cm depth and from the southern direction as reference values and estimated the effects of depth and direction on *K* by using a Bayesian linear mixed model.
...
When there are no observed *K* values for the focal depth and direction, we used the predicted median *K* values from the linear mixed model for the subsequent analysis (`r s_table("dir_dep_post")`).
...
We then modeled sapwood length as a function of DBH using a Bayesian linear model (`r s_fig("sap_dbh_points")`).
We used the posterior median of the model and interpolated DBH to estimate daily sapwood length (`r s_table("dbh_sawpwood_post")`).
...
Bayesian estimates of posterior medians and 95% credible intervals of the effects of diretions and depths on *K*.
...
The effects of directions were not signifincant.
...
`r s_table("full_segments_without_traits_table")`:
Bayesian estimates of posterior median, upper and lower credible intervals and effective sample sizes for multilevel model without traits, evaluated across different maximum applied pressure gradients.
...
```{r, echo=FALSE}
read_csv(here::here("data/full_segments_post.csv")) |>
  head(20) |>
  kbl(escape = FALSE) |>
  kable_classic()
...
`r s_table("all_seg_table")`: Bayesian estimates of posterior median, upper and lower credible intervals and effective sample sizes for multilevel model with all traits.
...
](`r here::here("figs/tr_scaled_bars2.png")`)

- sep_pool: model 1 (in the slide)
- sep_segments: model 2
- full_pool: model 3
- full_segments: model 4
- sep_granier: model 1 with Granier's coefficients
...
## species-specific model: pooled fitting for separte species

```{stan, file=here::here('stan/granier_without_traits_sp_pool.stan'), echo=TRUE, eval=FALSE, output.var="hoge2"}
```

## segment-separted fit model: multilevel fitting for separte species

```{stan, file=here::here('stan/granier_without_traits_sp_segments.stan'), echo=TRUE, eval=FALSE, output.var="hoge3"}
```

## species-pooled: pooled fitting for pooled species

```{stan, file=here::here('stan/granier_without_traits_full_pool.stan'), echo=TRUE, eval=FALSE, output.var="hoge4"}
```

## segement-pooled: multilevel fitting for pooled species

```{stan, file=here::here('stan/granier_without_traits_full_segments.stan'), echo=TRUE, eval=FALSE, output.var="hoge5"}
```

## segement-pooled with traits (with xylem type)

```{stan, file=here::here('stan/granier_with_traits.stan'), echo=TRUE, eval=FALSE, output.var="lkasd"}
```

## segement-pooled with traits without xylem type

```{stan, file=here::here('stan/granier_with_traits_no_xylem.stan'), echo=TRUE, eval=FALSE, output.var="m;lkm"}
```

```{r, echo=FALSE, eval=FALSE}
library(kableExtra)
tmp1 <- tar_read(fit_dir_dep_summary_no_temporal_hourly_dir)
tmp1 <- tmp1 |>
  janitor::clean_names() |>
...
tmp2 <- tar_read(fit_dir_dep_summary_no_temporal_hourly_dep)
tmp2 <- tmp2 |>
  janitor::clean_names() |>
  filter(str_detect(variable, "beta")) |>
  mutate(variable = case_when(
...
loo <- tar_read(loo_)
loo::loo_compare(loo[[1]], loo[[2]], loo[[3]], loo[[4]])

str(loo)

tar_read(fit_anova_log_summary_anova_noint) |> summary()
tar_read(fit_anova_summary_anova_noint_err) |> summary()
