...
This model is the most conventionl approach (stan code, `r s_table("species_only_table")`).
...
The 95% CI of the relationships between both K~s~ and VAF with coefficient *a* and *b* did not overlap with zero (Figs. `r fig_num("traits_points_main")`A, B, F, and G), when xylem-level variation was not included in the model.
However, when xylem-level variation was accounted for, only the relationship between Ks and coefficient *a* maintained a 95% CI that did not overlap with zero (Fig. `r fig_num("traits_points_main")`B)
