...
I put all the traits in the model at the same time, and the model with only log(ks) was the best model.
The other traits did not have effect on the coefficient *a* and *b*.
...
s4 <- withr::with_dir(rprojroot::find_root('_targets.R'),
...
stan_data <- withr::with_dir(rprojroot::find_root('_targets.R'),
  targets::tar_read(sap_trait_clean_ks))

xx <- seq(min(stan_data$xj[, 2]), max(stan_data$xj[, 2]), length = 80)
...
pred_points <- tibble(a_est, b_est, x = rescale(stan_data$xj[, 2]))
