...
d3 <- d2 |>
  ...
  summarize(tr = sum(value) / 600 * 1000) |>
  ungroup() |>
  mutate(pg = ifelse(model == "granier", "granier", pg)) |>
...
d2_ab <- d_ab |>
  pivot_longer(3:ncol(d_ab),
    names_to = "variable",
    values_to = "value") |>
  mutate(model = str_split_fixed(variable, "_", 5)[, 3]) |>
  mutate(ab = str_split_fixed(variable, "_", 5)[, 4]) |>
  mutate(model = paste(model, ab, sep = "_")) |>
  mutate(quantile = str_split_fixed(variable, "_", 5)[, 5])
...
d5 <- bind_rows(d4, d4_ab)

ggplot(d5, aes(x = pg, y = m, fill = model, group = model)) +
