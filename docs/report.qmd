...
```{stan, file=here::here('stan/sap_without_traits.stan'), echo=TRUE, eval=FALSE, output.var="hoge2"}
```
...
```{r}
library(ggridges)
tar_load(fit_ab_summary_grainer_without_traits)
tar_load(fit_ab_draws_grainer_without_traits)
...
sp_lab <- d |>
  dplyr::select(sp = species_name, xylem_type) |>
  unique() |>
...
# fit_ab_summary_sap_without_traits |>
...
draws <- fit_ab_draws_sap_without_traits |>
  janitor::clean_names()
