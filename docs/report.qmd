...
Values are the annual sap flux for each tree (Mg year^-1^ per tree).
...
Columns are:
 ...
 - x_h: 75% quantile
 - x_hh: 97.5% quantile
 - s_total_granier: estimated by Fd = 119*K^1.23

The table below shows the sum of all trees divided by 600 m^2^.
The unit is Kg m^-2^ year^-1^ (or mm year^-1^).
