...
```
  ...
  vector[Mj] real<lower=0> tau_alpha_l;
  for (j in 1:Mj) tau_alpha_l[j] = 2.5 * tan(tau_unif_alpha_l[j]);

  for (j in 1:Mj) {
    alpha_l[j] = mu_alpha_l_tilde[j] + tau_alpha_l[j] * theta_l[j];
  }
  for (k in 1:Nk) {
    alpha_k[j] = alpha_l[ll[k]] + tau_alpha_k[j] * theta_k[j];
  }
