...
d <- tar_read(imputed_full_df)
d2 <- tar_read(nonimputed_full_df)

a <- d |>
  filter(str_detect(date, "2015-02")) |>
  nrow()
...
tmp3 <- full_join(tmp, tmp2) |>
  ungroup() |>
  mutate(ratio = na_n / total_n) |>
  mutate(ym = str_c(year, month, sep = "-"))

ggplot(tmp3, aes(x = ym, y = na_n / total_n)) +
  geom_bar(stat = "identity")

sum(tmp3$na_n) / sum(tmp3$total_n)

tmp3 |>
  arrange(ratio)

tar_read(imputed_data_2015_1)$OOBerror
tar_read(imputed_data_2015_2)$OOBerror
tar_read(imputed_data_2015_3)$OOBerror
...
df_imp <- tar_read(imputed_full_df) |>
  mutate(date = paste(date, time)) |>
  mutate(date = ymd_hms(date))  |>
  mutate(year = year(date)) |>
  mutate(yday = yday(date))
df_mis <- tar_read(nonimputed_full_df) |>
  mutate(date = paste(date, time)) |>
  mutate(date = ymd_hms(date))  |>
  mutate(year = year(date)) |>
  mutate(yday = yday(date))
...
p <- df_imp |>
  dplyr::filter(yday <= 300) |>
  dplyr::filter(yday > 270) |>
  filter(year == 2015) |>
  ggplot(aes(x = date, y = ks, col = tree, lty = dir, shape = as.factor(dep))) +
  geom_point() +
  geom_line() +
  ggtitle("imputed")
ggsave("hoge.png", p, width = 50, height = 20, units = "cm")
...
p2 <- df_mis |>
  ...
  geom_line() +
  ggtitle("missing")
ggsave("hoge2.png", p2, width = 50, height = 20, units = "cm")
...
tar_read(imputed_data_2015_1)$OOBerror[1]

tar_read(imputed_data_2015_2)$OOBerror

tmp <- tar_read(combined_nrmse)
tmp2 <- tar_read(combined_nrmse_rest) |>
  mutate(id = str_extract(id, "(?<=imputed_df_).*$"))
bind_rows(tmp, tmp2) |>
  arrange(id)

input_string <- "nramse2_2016_9_imputed_df_2015_9"
str_extract(input_string, "(?<=imputed_df_).*$")

tar_load(nrmse_df)
hist(nrmse_df$NRMSE)

tar_read(imputed_rest_2016_10_imputed_df_2015_10)$OOBerror
tar_read(imputed_rest_2016_9_imputed_df_2015_9)$OOBerror
tar_read(imputed_rest_2016_10_imputed_df_2015_10)$OOBerror
...
Now I need DBH data. Can you send
values |>
...
tar_read(fit_ab_summary_granier_without_traits_full_segments_sap_all_clean_0.08)

d <- tar_read(imputed_full_df)

tmp <- tar_read(fit_ab_draws_granier_without_traits_full_segments_sap_all_clean_0.08) |>
  janitor::clean_names() |>
  dplyr::select(alpha_1_15, alpha_2_15, sigma)

calc_log_fd <- function(ks, log_a, b, sigma) {
  log_a + b * ks + sigma^2 / 2
}

d2 <- head(d, 200)

d |>
  mutate(log_fd = calc_log_fd(ks, tmp[1, 1], tmp[1, 2], tmp[1, 3])) |>
  mutate(fd = exp(log_fd))
```

```{r}
library(tidyverse)

d1 <- read_csv("data-raw/girth_increment.csv")
d2 <- read_csv("data-raw/initial_dbh.csv")
d3 <- read_csv("data-raw/sapwood_depth.csv")

plot(sapwood_depth ~ dbh, data = d3)
plot(sapwood_depth ~ dbh, log = "xy", data = d3)

fit1 <- lm(sapwood_depth ~ dbh, data = d3)
fit2 <- lm(log(sapwood_depth) ~ log(dbh), data = d3)
...
hist(d3$sapwood_depth)
hist(d3$dbh)

log(d3$sapwood_depth) |> hist()
log(d3$dbh) |> hist()

targets::tar_read(fit_dbh_sapwood_summary_normal)
...
```{r}
d3 <- read_csv("data-raw/sapwood_depth.csv")
# Defining the DBH range
x <- seq(min(log(d3$dbh)), max(log(d3$dbh)), length = 80)

# Reading the model fit
fit_draws <- targets::tar_read(fit_dbh_sapwood_draws_normal)

# Creating a nested data frame
nested_draws <- fit_draws %>%
  mutate(draw = row_number()) %>%
  nest(data = c(alpha, beta, sigma))

# Generating predictions for each draw and each x value
pred_df <- expand.grid(x = x, draw = 1:nrow(nested_draws)) %>%
  left_join(nested_draws, by = "draw") %>%
  mutate(pred_log = map2_dbl(data, x, ~.x$alpha + .x$beta * .y),
         pred = map2_dbl(data, pred_log, ~exp(.y + (.x$sigma^2 / 2))),  # correction here
         x = exp(x)) %>%
  select(-data)

# Calculating the median and 95% CI for each x value
...
ggplot(pred_summary, aes(x = x)) +
  geom_line(aes(y = median), color = "blue") +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.4) +
  labs(x = "DBH", y = "Sapwood length") +
  theme_bw()
```
...
I need to check the data for inital DBH measurement.
...
d1_long <- d1_re %>%
  ...
  mutate(date = lubridate::dmy(date), tree_id = as.numeric(str_replace(tree_id, "tree_", "")))
# Transform the initial DBH data frame to a long format
# d2 <- d2
...
rnorm(100) |> quantile(c(0, 0.025, 0.975))

dbh_data |> as.data.frame()

ggplot(dbh_data, aes(y = dbh, x = date, col = as.factor(tree_id))) +
  geom_point() +
  geom_line() +
  scale_y_log10() +
  facet_wrap(~ tree_id, ncol = 4)

dbh_data2 <- dbh_data |>
  janitor::clean_names() |>
  mutate(year = year(date)) |>
  mutate(yday = yday(date))  |>
  mutate(day = ifelse(year == 2016, yday + 365, yday))

fit1 <- lme4::lmer(log(dbh) ~ log(day) + (1 | tree_id), data = dbh_data2)
fit2 <- lme4::lmer(log(dbh) ~ day + (1 | tree_id), data = dbh_data2)
fit3 <- lme4::lmer(log(dbh) ~ date + (1 | tree_id), data = dbh_data2)
...
# Suppose these are your dates and DBH measurements for a single tree
dates <- c(1, 2, 3, 4, 5)  # these would be your actual date numbers
dbh <- c(10, 12, 14, 16, 18)

# Suppose you want to estimate DBH at date 2.5
date_to_predict <- 2.5

# Use the approx() function to perform linear interpolation
dbh_predicted <- approx(x = dates, y = dbh, xout = date_to_predict)$y

print(dbh_predicted)  # this should print 13.0
...
ggplot(dbh_data_interpolated, aes(y = dbh, x = date, col = as.factor(tree_id))) +
  # geom_point() +
  geom_line() +
  scale_y_log10() +
  facet_wrap(~ tree_id, ncol = 4)

imp_df <- tar_read(imputed_full_df)
...
# Create a sequence of times with increments of 10 minutes
time_seq <- seq(from = as.POSIXct("00:00:00", format = "%H:%M:%S", tz = "UTC"),
                to = as.POSIXct("23:50:00", format = "%H:%M:%S", tz = "UTC"),
                by = "10 min")
# Convert the sequence to 'HH:MM:SS' format
time_seq_formatted <- format(time_seq, format = "%H:%M:%S")

dbh_data_full <- dbh_data_interpolated |>
  mutate(dbh = map(dbh, rep, 144)) |>
  mutate(time = list(time_seq_formatted)) |>
  unnest(cols = c(dbh, time))

tmp <- imp_df |> head(2000)
...
fit1 <- lme4::lmer(ks ~ dir_fct + as.factor(dep) + (1 | tree), data = imp_df2)
fit2 <- lme4::lmer(log(ks) ~ dir_fct + as.factor(dep) + (1 | tree), data = imp_df2)

library(tictoc)
tic()
fit3 <- lme4::lmer(log(ks) ~ dir_fct + as.factor(dep) + (1 | tree) + (1 | date) + (1 |time), data = imp_df2)
toc()

tic()
fit4 <- imp_df2 |>
  mutate(tmp = paste(date, time)) %>%
  lme4::lmer(log(ks) ~ dir_fct + as.factor(dep) + (1 | tree) + (1 | tmp), data = .)
toc()

fit5 <- lm(log(ks) ~ dir_fct + as.factor(dep), data = imp_df2)
AIC(fit1, fit2, fit3, fit4, fit5)

summary(fit4)

hist(imp_df$ks)
summary(log(imp_df2$ks))
...
# d <- read_csv("data-raw/rubber_raw_data.csv")
# tail(d)
hoge <- left_join(imp_df, dbh_data_full, by = c("date", "time", "tree" = "tree_id"))

tmp <- imp_df |>
  dplyr::select(-dir_fct) |>
  pivot_wider(names_from = c(tree, dir, dep), values_from = ks)

tmp2 <- tmp %>%
  filter_at(vars(matches("t[0-9]")), all_vars(. != 0))

tmp2 |>
  pivot_longer(cols = matches("t[0-9]"), names_to = "tree", values_to = "ks")


```

When we want to calculate the 95% confidence interval of the product of two random variables, can we use the following method?

Can you make a time vector in R with increments of 10 minutes?
...
The upper qunatile of xx * yy is always smaller thant the product of the upper quantiles of xx and yy.
...
# bootstrap
set.seed(123)
bootstrap_samples <- replicate(2000, {
  indices <- sample(1:length(xx), replace = TRUE)
  quantile(xx[indices] * yy[indices], probs = c(0.025, 0.975))
})

# get 95% quantiles
lower_quantile <- quantile(bootstrap_samples[1,], 0.025)
upper_quantile <- quantile(bootstrap_samples[2,], 0.975)
...
Date is from 2015-01-01 to 2016-12-31.
tree_id is from 1 to 16.

```
[ins] r$> dbh_data_interpolated
# A tibble: 11,696 × 3
   tree_id date         dbh
     <dbl> <date>     <dbl>
 1       1 2015-01-01  18.0
...
imp_df2 <- imp_df |>
  ...
  mutate(dir_fct = as.factor(dir))

fit3 <- lme4::lmer(log(ks) ~ dir_fct + dep_fct + (1 | tree) + (1 | date), data = imp_df2)
...
summary(imp_df2)
imp_df2 |> pull(ks) |> log() |> hist()

xd <- model.matrix(log(ks) ~  dir_fct + dep_fct, data = imp_df2)

tmp <- list(
  N = nrow(imp_df2),
  K = ncol(xd),
  M = imp_df2 |> pull(tree) |> unique() |> length(),
  T = imp_df2 |> pull(time2) |> unique() |> length(),
  log_ks = log(imp_df2$ks),
  time = imp_df2 |> pull(time2),
  tree = imp_df2 |> pull(tree) |> as.factor() |> as.numeric(),
  x = xd,
  NULL
)
str(tmp)
...
tar_read(dir_dep_stan_data) |> str()
```
