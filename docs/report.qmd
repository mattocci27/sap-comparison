...
Using traditional method (Granier, 1985) and our new multilevel methods generate comparable a (intercept) but slightly different b (exponent) values (Figs. S5-6; Table S4). Based on variance portioning analysis, xylem types (80.77% of total variance) are the largest source for the variance for coefficient a, while segments (40.26% of total variance) are the largest source for the variance for coefficient b (Table S5). Generally, there was an increasing tendency for coefficient a and b values along with the increasing applied pressure gradient (Figs. S7-9)
...
Relationships between sap flux density and sap flow index K ((△T~max~-△T)/△T) for different upper limits of pressure gradients.
`p_g_lim` indicates the maximum pressure gradient for subsets of data used to draw posterior medians of the power-law relationships with inter-segment variation (i.e., multilevel approaches).
...
![coef_density2](`r here::here("figs/coef_density2.png")`)

Posterior distributions of coefficients *a* (A) and *b* (B) estimated by Bayesian multilevel models.
...
- Each point indicates each segment
- Vertical bars indicate 95% CIs
- Light and dark shaded regions indicate 95% and 50% CIs, respectively
- The 95% CI of the slope value for `r fig("traits_points")`B did not overlap with zero
- The 50% CI of the slope value for `r fig("traits_points")`D did not overlap with zero
