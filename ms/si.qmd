...
This model is the most conventional approach (stan code, `r s_table("species_only_table")`).

Second, the segment-inclusive model uses segments as its primary analytical unit.
While this model incorporates both inter-segment and interspecific variations, each species was fitted separately
(stan code, `r s_table("segments_inclusive_table")`).

Third, the species-xylem model takes species as its primary analytical unit, integrating interspecific and xylem-type variations
(stan code, `r s_table("species_xylem_table")`).
...
Similar to the species-xylem model, this model fits all species at the same time
(stan code, `r s_table("segments_xylem_table")`).
...
We also quantified the proportion of total variance attributed to each component (i.e., segments, species, and xylem types) in the coefficients *a* and *b* estimates, following the methodology described above.

Lastly, we built Bayesian hierarchical models incorporating wood traits ($\rho$, VAF, VF, and D~h~) and hydraulic trait *K~s~*.
Each of these traits was included separately as species-level predictors for the coefficients *a* and *b* and were integrated into our segment-xylem model
(stan code, `r s_table("traits_xylem")`).
Additionally, we built a variation of the segment-xylem model that excluded xylem-level variation
(stan code, `r s_table("traits_noxylem")`).
This allowed us to assess the effects of xylem-level variation on trait estimation for the coefficients *a* and *b*.
...
(showing only the first 10 rows here)
```{r, echo=FALSE}
...
Bayesian estimates of posterior median, upper and lower credible intervals, and effective sample sizes for the segments inclusive model, evaluated across different maximum applied pressure gradients.
...
**`r s_table("traits_xylem")`**: Bayesian estimates of posterior median, upper and lower credible intervals, and effective sample sizes for the trait-based model with xylem effects.
Details are the same as in `r s_table("species_only_table")`.
...
For example, trait `log_dh` and variable `beta_a[2,1]` indicate the effect of Dh on coefficient *a* for diffuse-porous trees on the log-scale.

(showing only the first 10 rows)

```{r, echo=FALSE}
read_csv(here::here("data/traits_xylem_post.csv")) |>
  head(10) |>
  kbl(escape = FALSE) |>
  kable_classic()
```

**`r s_table("traits_noxylem")`**: Bayesian estimates of posterior median, upper and lower credible intervals, and effective sample sizes for the trait-based model without xylem effects.
Details are the same as in `r s_table("traits_xylem")`.

```{r, echo=FALSE}
read_csv(here::here("data/traits_noxylem_post.csv")) |>
  head(10) |>
  kbl(escape = FALSE) |>
  kable_classic()
```

## Estimating tree transpiration in a forest stand

We developed a statistical framework to estimate tree transpiration while overcoming the challenges posed by missing data (*K*) and quantifying the uncertainty in our estimates.
Utilizing the missForest algorithm, we imputed missing *K* values.
Using Bayesian linear models, we estimated *K* values where depth and direction data were absent and derived sapwood area ($A_{sap}$) from DBH measurements.
This approach allows for a probabilistic assessment of the uncertainty of our estimates.

To implement this framework, we used the missForest algorithm [@Stekhoven2012] to handle missing K values in our dataset (`r s_fig("imp_points")`).
...
Because of power outage (?), we encountered an issue with an excess of missing *K* values in September, October, and December 2016.
The missing data from these periods could not be adequately imputed using only the data from the same months.
To address this, we used imputed data from the corresponding months in 2015 to impute the missing values for 2016.
...
In our dataset, *K* values were measured across three depths (0-2 cm, 2-4 cm, and 4-6 cm) and four directions (north, south, east, and west).
However, the dataset was incomplete as not all trees were measured for all depth-direction combinations.
To address this, we used the *K* values from the 0-2 cm depth and southern exposure as a baseline, employing a Bayesian linear model to estimate the effects of varying depths and directions on *K* values [(stan code)](#stan-codes).
This model enabled us to use actual observed *K* values for analysis when available and to substitute with the median predicted *K* values from the model when direct measurements were absent (`r s_table("dir_dep_post")`).
When the sapwood depth exceeded 6 cm, we adapted our approach for estimating *K* values by assigning them half the value of those measured at a depth of 4-6 cm from the same direction.

In addition to *K* value estimation, we conducted 16 measurements of DBH over two years.
...
This linear model of sapwood length is also crucial for calculating sap flux at 10-minute intervals ($S_{10min}$; g m^-2^ 10 min^-1^) using the equation
$S_{10min} = a \times K^b \times A_{sap} \times 600$
where the factor 600 is the number of seconds in 10-minute intervals.
...
We also evaluated the relative contribution of uncertainty in transpiration estimates by calculating the variance contributed
by the *a* and *b* coefficients ($\sigma^2_{\mathrm{ab}}$),
by the combined effects of direction and depth on *K* ($\sigma^2_{\mathrm{dir-dep}}$),
by direction ($\sigma^2_{\mathrm{dir}}$),
by depth ($\sigma^2_{\mathrm{dep}}$), and
by sapwood area ($\sigma^2_{\mathrm{sap}}$),
each time holding other variables at their median values.
These individual variances are then compared to the total variance obtained when considering posterior samples for all variables ($\sigma^2_{\mathrm{tol}}$).
The relative uncertainty due to *a* and *b* coefficients, for example, is quantified as the percentage of $\sigma^2_{\mathrm{ab}}$ to $\sigma^2_{\mathrm{tol}}$.
...
As an illustration, the posterior median *K* value at the 4-6 cm depth in the north is approximately 63% of the value at the 0-2 cm depth in the south direction.

```{r, echo=FALSE}
read_csv(here("data/dir_dep_comb_post.csv")) |>
  kbl(escape = FALSE) |>
  kable_classic()
```
...
- Pressure gradients (segments-xylem model)
- Variance partitioning (segments-xylem model)
- Model with traits (without xylem and with xylem)
...
Our segments-xylem model revealed little variance in the estimates of coefficients *a* and *b* between species (`r fig("coef_density")` and Table `r s_table_num("varpart_notrait")`).
...
The 95% CI of the relationships between both K~s~ and VAF with coefficients *a* and *b* did not overlap with zero (Figs. `r fig_num("traits_points_main")`A, B, F, and G), when the xylem-level variation was not included in the model.
When the xylem-level variation was accounted for, only the relationship between Ks and the coefficient *a* maintained a 95% CI that did not overlap with zero (Fig. `r fig_num("traits_points_main")`B).
However this model did not converge well because of the strong coveriance between Ks and xylem types (Fig. `r fig_num("traits_points_main")`B, `r s_table("traits_xylem")`)
...
](`r here::here("figs/tr_bar_ab.png")`)
...
Data for `r fig("scaled_tr_ab")`.
```{r}
read_csv(here("data/tr_bar_ab.csv"))
```
- tr_x: transpiration estimates
  - m: median
  - l: 2.5% quanatile
  - h: 97.5% quanatile
  - mean: mean
  - sd: sd
- pg: maximum applied pressure gradient
- model: model names to estimate coefficients *a* and *b*

![`r s_fig("scaled_tr_all")`:
This figure is not finalized.
](`r here::here("figs/tr_bar_all")`)

Data for `r fig("scaled_tr_all")`.

```{r}
read_csv(here("data/tr_bar_all.csv"))
```

- id: source of uncertainty
  - sarea: sapwood area
  - ab: a-b estiamtes from segments-xylem model at 0.08 MPa
  - dir_only: direction
  - dep_only: depth
  - dir_dep: covariance betwen direction and depth estimates
- tr_x: transpiration estimates
  - m: median
  - l: 2.5% quanatile
  - h: 97.5% quanatile
...
- tr_x: transpiration estimates
  ...
  - mean: mean
  - sd: sd
- var: variance of stand transpiration
- rel: relative contribution of each source of uncertainty
- rel_adj: adjusted relative contribution of each source of uncertainty so that the sum of relative contribution is 100%

## Similarity of transpiration rates across varying pressure gradients

(A)
Distribution of sap flow coefficient (K).
Data points correspond to K values for specific cardinal directions (South, West, East, North) and depths (0-2 cm, 2-4 cm, 4-6 cm) of an individual tree.
The dataset reflects 1-second intervals averaged over 10 minutes.
The distribution shows a higher frequency of data points with K values less than 0.05.
(B)
Sap flux density relationships under different pressure gradients (*P~g~*, MPa m^-1^).
Under *P~g~* = 0.02, the relationship between sap flux density and K is defined as Fd = 270K^1.09^.
Under *P~g~* = 0.08, the relationship is defined as Fd = 143K^0.717^.
At smaller K values, Fd is greater under *P~g~* = 0.02 than under *P~g~* = 0.08; this trend reverses at larger K values.
The cumulative sap flux density (i.e., transpiration) is likely comparable between the two *P~g~* values.
(C)
Sap flux density comparison at different *P~g~* values
Data points are sap flux (g s^-1^) under *P~g~* = 0.02 MPa m^-1^ vs. *P~g~* = 0.08 MPa m^-1^ for each tree recorded at 10-minute intervals.
The majority of data points indicate that under *P~g~* = 0.08, the sap flux is underestimated, while a subset with larger K values exhibit a pronounced overestimation of sap flux.
A color gradient indicates the sap flow coefficient (K), demonstrating its variation across the dataset.
(D)
Posterior distributions of coefficients *a* and *b* related to transpiration rates, under *P~g~* = 0.08.
Multiple regression analysis suggests that the sum of Fd (transpiration) is independent with a-b estiamtes (*P* > 0.05).
A color gradient is used to visualize annual transpiration rates.

```{r, include=FALSE, eval=FALSE}
ab_summarized_df <- tar_read(ab_summarized_df_species_xylem_post_ab_fit_draws_species_xylem_0.08)
ab_unce_df <- tar_read(ab_uncertainty_df_species_xylem_post_ab_fit_draws_species_xylem_0.08)
segments_post <- tar_read(segments_xylem_post_ab_fit_draws_segments_xylem_0.08)

library(ggridges)
tmp <- ab_unce_df |>
  group_by(id, tree) |>
  summarize(fd = mean(fd) * 1e-7) |>
  ungroup()

tmp2 <- tmp |>
  filter(id == "5")

ggplot(tmp, aes(x = fd, y = tree, fill = tree)) +
  geom_density_ridges() +
  geom_point(data = tmp2, aes(x = fd, y = tree))
...
![](`r here::here("figs/ab_example_panel.png")`)
...
## species-only model: pooled fitting for separate species

```{stan, file=here::here('stan/species_only.stan'), echo=TRUE, eval=FALSE, output.var="hoge2"}
```

## segment-inclusive model: multilevel fitting for separate species
