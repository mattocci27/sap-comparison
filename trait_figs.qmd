...
## Noxylem effect with colors

![](`r here::here("figs/traits_sp_points_main.png")`)

### Xylem

```{r, echo = FALSE}
render_eq <- function(summary, x, sp = FALSE) {
  beta_coef <- summary |>
    filter(str_detect(variable, "beta")) |>
    pull(q50)
  str_glue("a = {exp(beta_coef[1]) |> round(2)} {x}^{beta_coef[2] |> round(3)}") |> print()
  str_glue("b = {beta_coef[3] |> round(2)} + {beta_coef[4] |> round(3)} ln {x}") |> print()
}

# tmp <- tar_read(fit_summary_segments_noxylem_traits_simple_log_vaf)
# beta_coef <- tmp |>
#   filter(str_detect(variable, "beta")) |>
#   pull(q50)
# str_glue("a = {exp(beta_coef[1]) |> round(2)} Ks^{beta_coef[2] |> round(3)}")
# tr_glue("b = {beta_coef[3] |> round(2)} + {beta_coef[4] |> round(3)} ln Ks")
```

```{r, echo = FALSE}
tar_read(fit_summary_segments_noxylem_traits_simple_log_vaf) |> render_eq("VAF")
```

```{r, echo = FALSE}
tar_read(fit_summary_segments_noxylem_traits_simple_log_ks) |> render_eq("Ks")
```

### Species
```{r, echo = FALSE}
tar_read(fit_summary_segments_noxylem_traits_sp_simple_log_vaf) |> render_eq("VAF", sp = TRUE)
```

```{r, echo = FALSE}
tar_read(fit_summary_segments_noxylem_traits_sp_simple_log_ks) |> render_eq("Ks", sp = TRUE)
```
...
```{r}
fit_lm <- lm(log(y) ~ log(log_x), data = pub_df2)

fit_gmm <- nls(log(y) ~ log_a + b * log(log_x) - log(k + log_x^b),
               start = list(log_a = 1, b = 5, k = 800),
               data = pub_df2)

fit_weibull <- nls(log(y) ~ log_a + log(1 - exp(-b * log_x^k)),
               start = list(log_a = 1, b = 0.1, k = 1),
               data = pub_df2)

AIC(fit_lm, fit_gmm, fit_weibull)
```

### nonlog-scale

```{r}
fit_nl <- nls(y ~ a * (1 - b^log_x) + c,
  start = list(a = 4, b = 0.5, c = 1),
  data = pub_df2)

fit_nl2 <- nls(y ~ a * (1 - b^log_x),
  start = list(a = 2, b = 0.5),
  data = pub_df2)

fit_gmm2 <- nls(y ~ a * log_x^b / (k + log_x^b),
  start = list(a = 1.5, b = 5, k = 400),
  data = pub_df2)

AIC(fit_nl, fit_nl2, fit_gmm2)
```

`fit_nl` is the best model.

```{r, echo=FALSE}
aa <- coef(fit_nl)[1] |> round(2)
bb <- coef(fit_nl)[2] |> round(2)
cc <- coef(fit_nl)[3] |> round(2)
```

y = `r aa` * (1 - `r bb`^ln_x)  `r cc`
...
fitted_values <- fitted(fit_nl)

# Extract the actual values (log-transformed)
actual_values <- pub_df2$y
...
p <- ggplot(data, aes(x = x, y = y, color = group)) +
  geom_point(size = 12) +
  theme_minimal() +
  labs(title = "Plot with 6 Default Colors",
       x = "X Axis",
       y = "Y Axis",
       color = "Group")
...
p <- ggplot(data, aes(x = x, y = y, color = group)) +
  geom_point(size = 12) +
  theme_minimal() +
  labs(title = "Plot with 4 Default Colors",
       x = "X Axis",
       y = "Y Axis",
       color = "Group")
...
![](`r here::here("figs/ab_points_model4.png")`)
