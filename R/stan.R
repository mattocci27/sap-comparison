...
generate_sap_each_trait_no_xylem_stan_data_re2 <- function(data, trait_name) {
  d <- read_csv(data) |>
    filter(is.na(removed_k)) |>
    mutate(
      log_swc = log(swc),
...
generate_sap_each_trait_xylem_stan_data_re <- function(data, trait_name) {
  d <- read_csv(data) |>
    filter(is.na(removed_k)) |>
    mutate(
      log_swc = log(swc),
...
generate_sap_no_xylem_stan_data <- function(data, remove_abnormal_values = FALSE, upper_pressure = FALSE, traits = FALSE) {
  # library(tidyverse)
  # d <- read_csv("data/fd_k_traits.csv")
  d <- read_csv(data)
  ...
  # uk <- model.matrix(~ xylem_type, tmp)
  # uk[apply(uk, 1, sum) == 2, 1] <- 0
  n_sp <- d$species |> unique() |> length()
  # ul <- matrix(rep(1, 4), ncol = 4)
  uk <- matrix(rep(1, n_sp), ncol = n_sp)
...
#' @title with trait csv
write_with_traits_csv <- function(stan_summary) {
  s <- stan_summary
...
traits_points_main <- function(pred_data, use_color = FALSE) {
  ...
  p1 + p3 + p5 + p7 + p9 +
    ...
    theme(
      ...
      axis.title.y = element_text(size = 9, margin = margin(t = 0, r = 0, b = 0, l = 0)),
      # axis.text = element_text(size = 6),
      # plot.margin=unit(c(0.1, 0.05, 0.1, 0.05), "lines"),
      plot.margin=unit(c(0.1, 0.1, 0.1, 0.1), "lines"),
      plot.tag = element_text(size = 8)
    )
...
# Helper Functions
parse_variable <- function(var_col) {
  str_split_fixed(var_col, "\\[|\\]|,", 4)
}

predictor_extractor <- function(para, var) {
  para2 <- case_when(
...
prepare_data <- function(data) {
  read_csv(data) %>%
    filter(!is.na(wood_density), !is.na(swc), !is.na(dh), !is.na(vaf), !is.na(vf), !is.na(ks), is.na(removed_k))
}

generate_summary_trait_table <- function(fit_summary, data, xylem = TRUE) {
  d <- prepare_data(data)

  # Common preparations for data
  d_samp <- d %>%
    dplyr::select(species, xylem_type, sample_id) %>%
    arrange(xylem_type, sample_id) %>%
    mutate(j = as.numeric(as.factor(sample_id)),
           k = as.numeric(as.factor(species)),
           l = as.numeric(as.factor(xylem_type))) %>%
    unique() %>%
    mutate(level = "segments", target = sample_id)
  ...
  d_sp2 <- fit_summary %>%
    ...
    full_join(d_sp) %>%
    dplyr::select(variable, level, target, predictor, q50, q2.5, q97.5, rhat, ess_tail)

  d_samp2 <- fit_summary %>%
    filter(str_detect(variable, "^A\\[")) %>%
    mutate(para = parse_variable(variable)[, 2]) %>%
    mutate(j = parse_variable(variable)[, 3] |> as.numeric()) %>%
    full_join(d_samp) %>%
    mutate(predictor = ifelse(para == "1", "coefficient a", "coefficient b")) %>%
    dplyr::select(variable, level, target, predictor, q50, q2.5, q97.5, rhat, ess_tail)

  d_all <- fit_summary %>%
    filter(str_detect(variable, ifelse(xylem, "gamma", "beta"))) %>%
  ...
  result_data <- list(d_all, d_sp2, d_samp2)

  # Xylem-specific parts
  if (xylem) {
    d_xylem <- d_sp %>%
      dplyr::select(xylem_type, l) %>%
      unique() %>%
      mutate(level = "xylem types", target = xylem_type)

    d_xylem2 <- fit_summary %>%
      filter(str_detect(variable, "beta")) %>%
      dplyr::select(variable, q2.5, q97.5, q50, rhat, ess_tail) %>%
      mutate(para = parse_variable(variable)[, 2]) %>%
      mutate(predictor = predictor_extractor(para, variable)) %>%
      mutate(l = parse_variable(variable)[, 3] |> as.numeric()) %>%
      full_join(d_xylem) %>%
      dplyr::select(variable, level, target, predictor, q50, q2.5, q97.5, rhat, ess_tail) %>%
      mutate(target = case_when(
        target == "DP" ~ "Diffuse-porous tree",
    ...
    # result_data <- c(result_data, list(d_xylem2))
    result_data <- list(d_all, d_xylem2, d_sp2, d_samp2)
  }

  # Final combination and formatting
  bind_rows(result_data) %>%
    mutate(
      across(
        c(q50, q2.5, q97.5),
        ~ ifelse(
            variable %in% c("coefficient a", "intercept for coefficient a"),
            exp(.x),
            .x
          )
      )#,
    ...
    mutate(across(where(is.numeric), round, digits = 2),
           ess_tail = format(round(ess_tail, digits = 0), nsmall = 0),
           across(where(is.numeric), format, nsmall = 2)) %>%
...
add_parameter_names_sp2 <- function(df){
  df %>%
    mutate(
      param_name = case_when(
        # explicit beta entries
        variable == "gamma[1]"           ~ "intercept for coefficient a",
        variable == "gamma[2]"           ~ "effect of traits on coefficient a",
        variable == "gamma[3]"           ~ "intercept for coefficient b",
        variable == "gamma[4]"           ~ "effect of traits on coefficient b",
        str_detect(variable, "^beta\\[1,") ~ "intercept for coefficient a",
        str_detect(variable, "^beta\\[2,") ~ "effect of traits on coefficient a",
        str_detect(variable, "^beta\\[3,") ~ "intercept for coefficient b",
        str_detect(variable, "^beta\\[4,") ~ "effect of traits on coefficient b",

        str_detect(variable, "^alpha\\[1,") ~ "coefficient a",
        str_detect(variable, "^alpha\\[2,") ~ "coefficient b",
...
add_level_sp2 <- function(df){
  df %>%
    mutate(
      level_name = case_when(
        # explicit beta entries
        str_detect(variable, "^gamma\\[") ~ "overall",
        str_detect(variable, "^beta\\[") ~ "xylem",
        str_detect(variable, "^alpha\\[") ~ "species",
        str_detect(variable, "^A") ~ "segments",
        TRUE ~ NA_character_
      )
...
add_parameter_names_seg2 <- function(df){
  df %>%
    mutate(
      param_name = case_when(
        # explicit beta entries
...
generate_summary_trait_table_re2 <- function(fit_summary, data, sp = FALSE) {
  if (sp) {
    tmp <- fit_summary |>
      ...
      mutate(para1 = parse_variable(variable)[, 2] |> as.numeric()) %>%
      mutate(para2 = parse_variable(variable)[, 3] |> as.numeric()) %>%
      add_parameter_names_sp2() |>
      add_level_sp2()
  } else {
    tmp <- fit_summary |>
      filter(str_detect(variable, "^beta|^alpha\\[|A\\[|gamma")) |>
      mutate(para1 = parse_variable(variable)[, 2] |> as.numeric()) %>%
      mutate(para2 = parse_variable(variable)[, 3] |> as.numeric()) %>%
  ...
  bind_rows(d_all, d_xylem, d_sp, d_seg) |>
    dplyr::select(variable,　level_name, target, param_name, q50, q2.5, q97.5, rhat, ess_bulk) |>
    rename(
      variable_name = variable,
      level = level_name,
